...
ovalsize = 75; // sets number of pixels of circle diameter - need to change) Usually 10
GFPchannel = 1;// need to change depending on image
pERKchannel = 2;
tERKchannel = 3; 
pathname = "/Users/carolinewee/Desktop/pERK/Quantitative/auto/" //where you want to save the data -- will need to be modified
condition = "Chanpreet" // treatment that fish is exposed to e.g. "shock" 
...
// make new directory
File.makeDirectory(pathname + condition);
...
for (j=0; j<list.length; j++) {
    ...
    // close pERK channel. Assume GFP channel = 1
	...
	saveAs("Results",  pathname + condition + '/' + name + "-allcells.xls");
    ...
    run("Close All");
    saveAs("Results",  pathname + condition + '/' + name + "-GFP.xls");
}
